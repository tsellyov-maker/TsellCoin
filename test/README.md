This directory contains integration tests that test tsellcoind and its
...
tsellcoind and tsellcoin-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, TsellCoin Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the tsellcoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another tsellcoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its tsellcoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other tsellcoind processes are running.

On linux, the test framework will warn if there is another
tsellcoind process running when the tests are started.

If there are zombie tsellcoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
tsellcoind processes running on the system, so should not be used if any non-test
tsellcoind processes are being run.**
...
sure tsellcoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and tsellcoind `debug.log`s will all be dumped
...
`test_framework.log` and tsellcoind `debug.log`s can be combined into a single
...
call methods that interact with the tsellcoind nodes-under-test.

If further introspection of the tsellcoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/tsellcoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/tsellcoind <pid>
```
