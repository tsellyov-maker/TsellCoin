...
Tracing (USDT) support for TsellCoin Core on Linux using. For more information on
USDT support in TsellCoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `tsellcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `tsellcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[TsellCoin Core USDT documentation] on how to list available tracepoints in your
`tsellcoind` binary.

[TsellCoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof tsellcoind)
```
...
TsellCoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof tsellcoind)
```
...
In a different terminal, starting TsellCoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/tsellcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof tsellcoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof tsellcoind)
```
