...
gen-tsellcoin-conf.sh
===================

Generates a tsellcoin.conf file in `share/examples/` by parsing the output from `tsellcoind --help`. This script is run during the
release process to include a tsellcoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-tsellcoin-conf.sh
```
