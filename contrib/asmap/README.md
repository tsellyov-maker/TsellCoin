...
be used in TsellCoin Core with the `-asmap` option.
...
to binary before being used with TsellCoin Core.
...
the `tsellcoin-cli getnodeaddresses` command.
...
```
tsellcoin-cli getnodeaddresses 0 > addrs.json
```
