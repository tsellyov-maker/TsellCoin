# Bootstrappable TsellCoin Core Builds

This directory contains the files necessary to perform bootstrappable TsellCoin
Core builds.
...
To build TsellCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      TsellCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/tsellcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/tsellcoin-core/guix.sigs).
