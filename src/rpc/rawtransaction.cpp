                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in tsellcoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in tsellcoin-common, so we query them here and push the
...
static RPCMethod decodepsbt()
{
    return RPCMethod{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed TsellCoin transaction.",
                {
...
static RPCMethod combinepsbt()
{
    return RPCMethod{
        "combinepsbt",
        "Combine multiple partially signed TsellCoin transactions into one transaction.\n"
