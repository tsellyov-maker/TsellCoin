...
static RPCMethod echo(const std::string& name)
{
    return RPCMethod{
        ...
        "Simply echo back the input arguments. This command is for testing.\n"
                ...
                "tsellcoin-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCMethod{
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new tsellcoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new tsellcoin-echo executable,
                // and spawn tsellcoin-echo below instead of tsellcoin-node. But
                // using tsellcoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("tsellcoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a tsellcoind
                // process not a tsellcoind-node process, so just create a local
