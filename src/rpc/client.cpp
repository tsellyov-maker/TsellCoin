...
//! Specify whether parameter should be parsed by tsellcoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make tsellcoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `tsellcoin-cli` attempt to parse the
 ...
 * `tsellcoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make tsellcoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `tsellcoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so tsellcoin-cli
 ...
 *
 * For example, the command `tsellcoin-cli -named createwallet "my=wallet"`,
