...
namespace init {
namespace {
const char* EXE_NAME = "tsellcoin-node";

class TsellCoinNodeInit : public interfaces::Init
{
public:
    TsellCoinNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
{
    auto init = std::make_unique<init::TsellCoinNodeInit>(node, argc > 0 ? argv[0] : "");
    // Check if tsellcoin-node is being invoked as an IPC server. If so, then
