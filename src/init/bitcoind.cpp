...
namespace init {
namespace {
const char* EXE_NAME = "tsellcoind";

class TsellCoindInit : public interfaces::Init
{
public:
    TsellCoindInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::TsellCoindInit>(node);
}
} // namespace interfaces
