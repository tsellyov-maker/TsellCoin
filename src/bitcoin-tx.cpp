...
static void SetupTsellCoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " tsellcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The tsellcoin-tx tool is used for creating and modifying tsellcoin transactions.\n\n"
                "tsellcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded tsellcoin transaction, or with \"-create [commands]\" to create a hex-encoded tsellcoin transaction.\n"
                "\n"
                "Usage: tsellcoin-tx [options] <hex-tx> [commands]\n"
                "or:    tsellcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tsellcoin transaction
