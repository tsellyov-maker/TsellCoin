...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, TsellCoinUnit unit)
{
    int numBlocks;
    ...
    if (wtx.is_coinbase && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += TsellCoinUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (all_from_me)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (all_to_me)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn& txin : wtx.tx->vin) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, -wallet.getDebit(txin)) + "<br>";
                }
            }
            mine = wtx.txout_is_mine.begin();
            for (const CTxOut& txout : wtx.tx->vout) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Credit") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, wallet.getCredit(txout)) + "<br>";
                }
            }
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + TsellCoinUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal tsellcoin:URI (tsellcoin:123...?message=example)
            ...
            if (auto prev{node.getUnspentOutput(prevout)}) {
                {
                    ...
                    strHTML = strHTML + " " + tr("Amount") + "=" + TsellCoinUnits::formatHtmlWithUnit(unit, vout.nValue);
