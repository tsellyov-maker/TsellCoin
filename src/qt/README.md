This directory contains the source code for the TsellCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the TsellCoin UI.
...
- ClientModel is used by the main application `tsellcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a tsellcoin:... link from a browser).
...
* UI elements like TsellCoinAmountField, which inherit from QWidget.
* `tsellcoinstrings.cpp`: automatically generated
* `tsellcoinunits.(h/cpp)`: TSC / mTSC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/tsellcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `tsellcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `tsellcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `tsellcoin-qt`
9. You're all set! Start developing, building, and debugging the TsellCoin Core GUI
