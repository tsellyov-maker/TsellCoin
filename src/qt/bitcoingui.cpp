 ...
 *
 * Ref: https://github.com/tsellcoin/tsellcoin/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string TsellCoinGUI::DEFAULT_UIPLATFORM =
...
TsellCoinGUI::TsellCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &TsellCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &TsellCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &TsellCoinGUI::showModalOverlay);
...
void TsellCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a TsellCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tsellcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &TsellCoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &TsellCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &TsellCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your TsellCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified TsellCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed TsellCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed TsellCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a tsellcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible TsellCoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &TsellCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &TsellCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &TsellCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &TsellCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &TsellCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &TsellCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &TsellCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &TsellCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &TsellCoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &TsellCoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &TsellCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &TsellCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &TsellCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &TsellCoinGUI::showDebugWindow);
}

void TsellCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void TsellCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &TsellCoinGUI::setCurrentWalletBySelectorIndex);
...
void TsellCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &TsellCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &TsellCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &TsellCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &TsellCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void TsellCoinGUI::enableHistoryAction(bool privacy)
{
...
void TsellCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &TsellCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &TsellCoinGUI::removeWallet);
...
WalletController* TsellCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void TsellCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &TsellCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &TsellCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &TsellCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &TsellCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &TsellCoinGUI::incomingTransaction);
    connect(this, &TsellCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void TsellCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void TsellCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void TsellCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void TsellCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TsellCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void TsellCoinGUI::createTrayIcon()
{
...
void TsellCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &TsellCoinGUI::toggleHidden);
...
void TsellCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void TsellCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void TsellCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void TsellCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void TsellCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void TsellCoinGUI::openClicked()
{
...
void TsellCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TsellCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void TsellCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void TsellCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TsellCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TsellCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void TsellCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void TsellCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to TsellCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void TsellCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void TsellCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void TsellCoinGUI::updateHeadersSyncProgressLabel()
{
...
void TsellCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void TsellCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &TsellCoinGUI::quitRequested);
...
void TsellCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void TsellCoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &TsellCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void TsellCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void TsellCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &TsellCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &TsellCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void TsellCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TsellCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TsellCoinGUI::incomingTransaction(const QString& date, TsellCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(TsellCoinUnits::formatWithUnit(unit, amount, true));
...
void TsellCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TsellCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TsellCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TsellCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TsellCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void TsellCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TsellCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void TsellCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void TsellCoinGUI::updateWindowTitle()
{
...
void TsellCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TsellCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TsellCoinGUI::detectShutdown()
{
...
void TsellCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void TsellCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(TsellCoinGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = TsellCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void TsellCoinGUI::subscribeToCoreSignals()
{
...
void TsellCoinGUI::unsubscribeFromCoreSignals()
{
...
bool TsellCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<TsellCoinUnit> units = TsellCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const TsellCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, TsellCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const TsellCoinUnit u : TsellCoinUnits::availableUnits()) {
        menu->addAction(TsellCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(TsellCoinUnit newUnits)
{
    setText(TsellCoinUnits::longName(newUnits));
}
