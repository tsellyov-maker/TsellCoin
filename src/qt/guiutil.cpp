...
#include <qt/tsellcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TsellCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TsellCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new TsellCoinAddressCheckValidator(parent));
}
...
bool parseTsellCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tsellcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("tsellcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!TsellCoinUnits::parse(TsellCoinUnit::TSC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseTsellCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTsellCoinURI(uriInstance, out);
}

QString formatTsellCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tsellcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TsellCoinUnits::format(TsellCoinUnit::TSC, info.amount, false, TsellCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openTsellCoinConf()
{
    ...
    /* Open tsellcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TsellCoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TsellCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("TsellCoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for TsellCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "tsellcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("tsellcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a tsellcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=TsellCoin\n";
        else
            optionFile << strprintf("Name=TsellCoin (%s)\n", ChainTypeToString(chain));
