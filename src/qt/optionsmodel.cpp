...
/** Call node.updateRwSetting() with TsellCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because TsellCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/tsellcoin/tsellcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because tsellcoin 22.x will not try to read these.
...
//! Convert enabled/size values to tsellcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from tsellcoin -prune setting.
...
//! Get pruning size value to show in GUI from tsellcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayTsellCoinUnit")) {
        settings.setValue("DisplayTsellCoinUnit", QVariant::fromValue(TsellCoinUnit::TSC));
    }
    QVariant unit = settings.value("DisplayTsellCoinUnit");
    if (unit.canConvert<TsellCoinUnit>()) {
        m_display_tsellcoin_unit = unit.value<TsellCoinUnit>();
    } else {
        m_display_tsellcoin_unit = TsellCoinUnit::TSC;
        settings.setValue("DisplayTsellCoinUnit", QVariant::fromValue(m_display_tsellcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if tsellcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_tsellcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<TsellCoinUnit>() == m_display_tsellcoin_unit) return;
    m_display_tsellcoin_unit = new_unit.value<TsellCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayTsellCoinUnit", QVariant::fromValue(m_display_tsellcoin_unit));
    Q_EMIT displayUnitChanged(m_display_tsellcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/tsellcoin/tsellcoin/pull/8273
    ...
    // (https://github.com/tsellcoin-core/gui/issues/567).
