...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<TsellCoinUnit>("TsellCoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. tsellcoin_de.qm (shortcut "de" needs to be defined in tsellcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. tsellcoin_de_DE.qm (shortcut "de_DE" needs to be defined in tsellcoin.qrc)
...
static const char* qt_argv = "tsellcoin-qt";

TsellCoinApplication::TsellCoinApplication()
...
void TsellCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the TsellCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", TsellCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void TsellCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool TsellCoinApplication::createOptionsModel(bool resetSettings)
{
...
void TsellCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new TsellCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &TsellCoinGUI::quitRequested, this, &TsellCoinApplication::requestShutdown);
...
void TsellCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void TsellCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool TsellCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void TsellCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &TsellCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &TsellCoinApplication::handleRunawayException);
    connect(this, &TsellCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &TsellCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void TsellCoinApplication::parameterSetup()
{
...
void TsellCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void TsellCoinApplication::requestInitialize()
{
...
void TsellCoinApplication::requestShutdown()
{
...
void TsellCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // tsellcoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TsellCoinGUI::handlePaymentRequest);
        connect(window, &TsellCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void TsellCoinApplication::handleRunawayException(const QString &message)
{
...
void TsellCoinApplication::handleNonFatalException(const QString& message)
{
...
WId TsellCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool TsellCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", TsellCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(tsellcoin);
    Q_INIT_RESOURCE(tsellcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see tsellcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see tsellcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse tsellcoin.conf, determine network, switch to network specific
    ...
    // tsellcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
