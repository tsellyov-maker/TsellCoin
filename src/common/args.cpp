...
const char * const BITCOIN_CONF_FILENAME = "tsellcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //tsellcoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\TsellCoin
    //   new: C:\Users\Username\AppData\Local\TsellCoin
    // macOS: ~/Library/Application Support/TsellCoin
    // Unix-like: ~/.tsellcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "TsellCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "TsellCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/TsellCoin";
#else
    // Unix-like
    return pathRet / ".tsellcoin";
#endif
#endif
}
