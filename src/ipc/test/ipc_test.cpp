// Copyright (c) 2023-present The TsellCoin Core developers
...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_tsellcoin", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_tsellcoin", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_tsellcoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_tsellcoin", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_TsellCoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_tsellcoin.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("tsellcoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("tsellcoin_sock1_XXXXXX")),
    };
