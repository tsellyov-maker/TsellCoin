// Copyright (c) 2025-present The TsellCoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries tsellcoin-node, tsellcoin-gui.
  -M, --monolithic       Run monolithic binaries tsellcoind, tsellcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'tsellcoin-qt [ARGS]' or 'tsellcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'tsellcoind [ARGS]' or 'tsellcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'tsellcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'tsellcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'tsellcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_tsellcoin [ARGS]'.
  chainstate [ARGS] Run tsellcoin kernel chainstate util, equivalent to running 'tsellcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_tsellcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_tsellcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "tsellcoin-gui" : "tsellcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "tsellcoin-node" : "tsellcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("tsellcoin-cli");
            // Since "tsellcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("tsellcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("tsellcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_tsellcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("tsellcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_tsellcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_tsellcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("tsellcoin-util");
        } else {
...
//! Execute the specified tsellcoind, tsellcoin-qt or other command line in `args`
...
//!             be a relative path to a tsellcoind, tsellcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/tsellcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/tsellcoin/tsellcoin/pull/31375#discussion_r1861814807)
