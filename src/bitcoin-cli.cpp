...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: tsellcoin-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to tsellcoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The tsellcoin-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: tsellcoin-cli [options] <command> [params]\n"
                "or:    tsellcoin-cli [options] -named <command> [name=value]...\n"
                "or:    tsellcoin-cli [options] help\n"
                "or:    tsellcoin-cli [options] help <command>\n"
                "\n";
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        if (!reply["error"].isNull()) {
            if (reply["error"]["code"].getInt<int>() == RPC_METHOD_NOT_FOUND) {
                throw std::runtime_error("-addrinfo requires tsellcoind v26.0 or later which supports getaddrmaninfo RPC. Please upgrade your node or use tsellcoin-cli from the same version.");
            }
            return reply;
        }
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (const auto res{ToIntegral<uint8_t>(args.at(0))}) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo level argument: %s\nFor more information, run: tsellcoin-cli -netinfo help", args.at(0)));
            }
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    m_outbound_only_selected = true;
                } else if (n) {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: tsellcoin-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: tsellcoin-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 209900) {
            throw std::runtime_error("-netinfo requires tsellcoind server to be running v0.21.0 and up");
        }
    ...
    const std::string m_help_doc{
        ...
        "> tsellcoin-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> tsellcoin-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> tsellcoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "Full dashboard, but with outbound peers only\n"
        + strprintf("> tsellcoin-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title tsellcoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "See this help\n"
        "> tsellcoin-cli -netinfo help\n"};
};
...
static std::optional<UniValue> CallIPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::string& endpoint, const std::string& username)
{
    ...
    std::unique_ptr<interfaces::Init> local_init{interfaces::MakeBasicInit("tsellcoin-cli")};
    if (!local_init || !local_init->ipc()) {
        if (ipcconnect == "auto") return {}; // Use HTTP if -ipcconnect=auto is set and there is no IPC support.
        throw std::runtime_error("tsellcoin-cli was not built with IPC support");
    }
    ...
    try {
        ...
        throw CConnectionFailed{strprintf("%s\n\n"
            "Probably tsellcoin-node is not running or not listening on a unix socket. Can be started with:\n\n"
            "    tsellcoin-node -chain=%s -ipcbind=unix", e.what(), gArgs.GetChainTypeString())};
    }
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::string& endpoint, const std::string& username)
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the tsellcoind server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"tsellcoin-cli -help\" for more info.",
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            ...
            strPrint += " (run \"tsellcoin-cli -h\" for help or \"tsellcoin-cli listwallets\" to see which wallets are currently loaded).";
        }
    } else {
