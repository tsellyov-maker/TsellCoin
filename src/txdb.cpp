    ...
    if (old_tip.IsNull()) {
        ...
        if (old_heads.size() == 2) {
            if (old_heads[0] != block_hash) {
                LogError("The coins database detected an inconsistent state, likely due to a previous crash or shutdown. You will need to restart tsellcoind with the -reindex-chainstate or -reindex configuration option.\n");
            }
