# Fuzzing TsellCoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing TsellCoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/tsellcoin/tsellcoin
$ cd tsellcoin/
...
## Overview of TsellCoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/tsellcoin_report.pdf) includes an external overview of the status of TsellCoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `tsellcoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`tsellcoin-core/qa-assets`](https://github.com/tsellcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`tsellcoin-core/qa-assets`](https://github.com/tsellcoin-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/tsellcoin-core/qa-assets
...
an instrumentalized libc++, then using it to build [TsellCoin Core dependencies
from source](../depends/README.md) and finally the TsellCoin Core fuzz binary
...
the qa-assets repo
(https://github.com/tsellcoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`tsellcoin-core/qa-assets`](https://github.com/tsellcoin-core/qa-assets) repo.

Every single pull request submitted against the TsellCoin Core repo is automatically tested against all inputs in the [`tsellcoin-core/qa-assets`](https://github.com/tsellcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make TsellCoin Core more robust.
...
# Fuzzing TsellCoin Core using afl++

## Quickstart guide

To quickly get started fuzzing TsellCoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing TsellCoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing TsellCoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
TsellCoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/tsellcoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=tsellcoin-core%20status:open).

TsellCoin Core follows its [security disclosure policy](https://tsellcoincore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_tsellcoin-core/latest).
