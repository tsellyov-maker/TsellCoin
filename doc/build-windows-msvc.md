...
This guide describes how to build tsellcoind, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
The TsellCoin Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the TsellCoin Core project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to TsellCoin Core.
...
can occur if your local TsellCoin Core repository path contains spaces, you can override the vcpkg install directory
...
To improve the build process performance, one might add the TsellCoin repository directory to the Microsoft Defender Antivirus exclusions.
