TsellCoin Core version 29.0 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-29.0/>
...
previously TsellCoin Core would listen for incoming Tor connections on `127.0.0.1:8334`.
...
127.0.0.1:5556`, or configure tsellcoind with `-bind=127.0.0.1:8334=onion` to get the previous behavior.
(#31223)
...
- As a safety check, TsellCoin core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. TsellCoin Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
3. CMake variables may be used to configure the build system. **Some defaults have changed.** For example, you will now need to add `-DWITH_ZMQ=ON` to build with zmq and `-DBUILD_GUI=ON` to build `tsellcoin-qt`. See [Autotools to CMake Options Mapping](https://github.com/tsellcoin-core/tsellcoin-devwiki/wiki/Autotools-to-CMake-Options-Mapping) for details.
4. For single-configuration generators, the default build configuration (`CMAKE_BUILD_TYPE`) is "RelWithDebInfo". However, for the "Release" configuration, CMake defaults to the compiler optimization flag `-O3`, which has not been extensively tested with TsellCoin Core. Therefore, the build system replaces it with `-O2`.
...
```
cmake -B build
cmake --build build --target tsellcoind
cmake --install build --component tsellcoind
```
...
- A new tool [`utxo_to_sqlite.py`](https://github.com/tsellcoin/tsellcoin/blob/v29.0/contrib/utxo-tools/utxo_to_sqlite.py)
