...
TsellCoin Core version v30.0 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-30.0/>
...
TsellCoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. TsellCoin
...
New `tsellcoin` command
---------------------

- A new `tsellcoin` command line tool has been added to make features more discoverable
  and convenient to use. The `tsellcoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `tsellcoin node` is a synonym for
  `tsellcoind`, `tsellcoin gui` is a synonym for `tsellcoin-qt`, and `tsellcoin rpc` is a synonym
  for `tsellcoin-cli -named`. Other commands and options can be listed with `tsellcoin help`.
  The new `tsellcoin` command is an alternative to calling other commands directly, but it
...
- The new `tsellcoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `tsellcoin -m node -ipcbind=unix` it will
  ...
  (`tsellcoin-node` instead of `tsellcoind`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/tsellcoin/tsellcoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/tsellcoin/tsellcoin/blob/master/doc/multiprocess.md)),
...
- The `test_tsellcoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `tsellcoin` command
  as `tsellcoin test`. The `libexec/` directory also contains new `tsellcoin-node` and
  `tsellcoin-gui` binaries which support IPC features and are called through the
  `tsellcoin` tool. In source builds only, `test_tsellcoin-qt`, `bench_tsellcoin`, and
  `tsellcoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `tsellcoin` command. See `tsellcoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of TsellCoin Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in TsellCoin Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the tsellcoin-wallet tool,
