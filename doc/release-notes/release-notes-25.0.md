...
TsellCoin Core version 25.0 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/tsellcoin/tsellcoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `tsellcoin-cli` tool both take advantage of this, so
  for example:

```sh
tsellcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
tsellcoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, TsellCoin Core will now return an error at startup. (#25574)
...
before TsellCoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching tsellcoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/tsellcoin/tsellcoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
