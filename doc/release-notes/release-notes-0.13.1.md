TsellCoin Core version 0.13.1 is now available from:

  <https://tsellcoin.org/bin/tsellcoin-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a tsellcoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of TsellCoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/tsellcoin/tsellcoin/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming TsellCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for TsellCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each TsellCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of tsellcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of tsellcoins being spent is exactly
...
- **Increased security for multisig:** TsellCoin addresses (both P2PKH addresses
  ...
  believed to be provided by TsellCoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  TsellCoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of TsellCoin's current ledger (called the UTXO set),
  ...
  note that TsellCoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  TsellCoin users to individually opt-in to almost any change in the TsellCoin
  Script language when those users receive new transactions.  Features
  currently being researched by TsellCoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other TsellCoin community members are studying several other improvements
...
[Segwit FAQ]: https://tsellcoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://tsellcoincore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/tsellcoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/tsellcoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/tsellcoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/tsellcoin/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://tsellcoincore.org/en/2016/06/08/version-bits-miners-faq/
...
Since TsellCoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/tsellcoin/bips/blob/master/bip-0147.mediawiki
...
- `tsellcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `tsellcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in tsellcoinconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.tsellcoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libtsellcoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_tsellcoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add tsellcoin-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` tsellcoin-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_tsellcoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name tsellcoin.qrc  (tsellcoinsSG)
...
- #8817 `bcf3806` update tsellcoin-tx to output witness data (jnewbery)
