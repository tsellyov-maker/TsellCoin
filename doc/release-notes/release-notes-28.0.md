TsellCoin Core version 28.0 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-28.0/>
...
Running TsellCoin Core binaries on macOS requires self signing.
```
cd /path/to/tsellcoin-28.0/bin
xattr -d com.apple.quarantine tsellcoin-cli tsellcoin-qt tsellcoin-tx tsellcoin-util tsellcoin-wallet tsellcoind test_tsellcoin
codesign -s - tsellcoin-cli tsellcoin-qt tsellcoin-tx tsellcoin-util tsellcoin-wallet tsellcoind test_tsellcoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. TsellCoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/tsellcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\TsellCoin`
to `C:\Users\Username\AppData\Local\TsellCoin`. TsellCoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/tsellcoin/tsellcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libtsellcoinconsensus Removal
---------------------------

The libtsellcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if TsellCoin Core was listening for P2P connections, either using
...
- TsellCoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/tsellcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running TsellCoin Core with the configuration
...
are now required to compile TsellCoin Core. (#29091, #30263)

- The minimum required glibc to run TsellCoin Core is now
...
- A new `-testdatadir` option has been added to `test_tsellcoin` to allow specifying the
...
Previous releases of TsellCoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
