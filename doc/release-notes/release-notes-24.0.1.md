...
TsellCoin Core version 24.0.1 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-24.0.1/>
...
This version of TsellCoin Core adds a new `mempoolfullrbf` configuration
...
Some TsellCoin services today expect that the first version of an
...
The TsellCoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [TsellCoin Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to TsellCoin Core are
...
[TsellCoin Core 0.12.0]: https://tsellcoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
[bip125]: https://github.com/tsellcoin/bips/blob/master/bip-0125.mediawiki
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the TsellCoin Core wallet yet.
  You can find more about Miniscript on the [reference website](https://tsellcoin.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the TsellCoin Core wallet, change output
...
[documentation](https://github.com/tsellcoin/tsellcoin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the tsellcoin GUI (such as the pruning setting,
proxy settings, UPNP preferences) are now saved to `<datadir>/settings.json`
file rather than to the Qt settings backend (windows registry or unix desktop
config files), so these settings will now apply to tsellcoind, instead of being
ignored. (#15936, gui#602)

- Also, the interaction between GUI settings and `tsellcoin.conf` settings is
simplified. Settings from `tsellcoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`tsellcoin.conf` values. (#15936)
