...
TsellCoin Core version 0.20.0 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from TsellCoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since TsellCoin Core version 0.18.0.
...
- Testing or debugging of implementations of the TsellCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of TsellCoin Core. TsellCoin Core logs debug messages
...
- OpenSSL is no longer used by TsellCoin Core.  (#17265)

- BIP70 support has been fully removed from TsellCoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/tsellcoin/tsellcoin/issues/16599),
  [PR #16702](https://github.com/tsellcoin/tsellcoin/pull/16702), and the
  `tsellcoind help` for more information.  This option is experimental and
...
- All custom settings configured when TsellCoin Core starts are now
...
- The "Start TsellCoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://tsellcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple TsellCoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed TsellCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/tsellcoin-core/HWI) for
  signing.  Future versions of TsellCoin Core should support a GUI option
...
- TsellCoin Core's automatically-generated source code documentation is
  now available at https://doxygen.tsellcoincore.org.  (#17596)
...
- The `tsellcoin-cli` utility used with the `-getinfo` parameter now
...
- The `tsellcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `tsellcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_tsellcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_tsellcoin` binary (hebasto)
...
- #18702 Fix ASLR for tsellcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate tsellcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest tsellcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_tsellcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (tsellcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone tsellcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about TsellCoin Core files (hebasto)
...
- #17119 Fix broken tsellcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from tsellcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.tsellcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to tsellcoin-wallet (hel-o)
...
- #18341 Replace remaining literal TSC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On tsellcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from tsellcoin-cli -getinfo (malevolent)
