...
TsellCoin Core version 23.0 is now available from:

  <https://tsellcoincore.org/bin/tsellcoin-core-23.0/>
...
- A tsellcoind node will no longer rumour addresses to inbound peers by default.
  They will become eligible for address gossip after sending an ADDR, ADDRV2,
  or GETADDR message. (#21528)

- Before this release, TsellCoin Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, TsellCoin nodes listening on non-standard ports would likely not get any TsellCoin Core peers connecting to them. This preference has been removed. (#23542)

- Full support has been added for the CJDNS network. See the new option `-cjdnsreachable` and [doc/cjdns.md](https://github.com/tsellcoin/tsellcoin/tree/23.x/doc/cjdns.md) (#23077)
...
TsellCoin Core release binaries for Linux now include experimental tracepoints which
...
[doc/tracing.md](https://github.com/tsellcoin/tsellcoin/blob/23.x/doc/tracing.md) and
usage examples are provided in [contrib/tracing/](https://github.com/tsellcoin/tsellcoin/tree/23.x/contrib/tracing).
...
- The top-level fee fields `fee`, `modifiedfee`, `ancestorfees` and `descendantfees`
  ...
  fields in the `fees` object are denominated in TSC. (#22689)
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. TsellCoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from TsellCoin Core in 22.0. (#22544)
