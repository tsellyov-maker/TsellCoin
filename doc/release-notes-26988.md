...
- CLI -addrinfo now returns the full set of known addresses. In previous versions (v22.0 - v30.0) the set of returned
  ...
  to connect to, which does not filter. Note: CLI -addrinfo now requires tsellcoind v26.0 or later, as it uses the
  getaddrmaninfo RPC internally. Users querying older, unmaintained node versions would need to use an older tsellcoin-cli
  version. (#26988)
